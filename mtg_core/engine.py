...
class MTGEngine:
    ...
    def _resolve_pass_priority(self, player_id: str) -> Dict[str, Any]:
        ...
        if self._pass_streak >= 2:
            ...
            if self.game.zones.stack:
                ...
                # Priority always resets to active player after an item resolves from the stack
        ...
        self._priority_holder = self.opponents(player_id)[0]
    ...
    def _end_turn(self) -> None:
        ...
        if self.game.extra_turns:
            ...
            t.active_player_id = self.opponents(t.active_player_id)[0]
        t.phase = Phase.BEGINNING
        t.step = Step.UNTAP
        t.attackers = []
        t.blockers = {}
    ...
    def opponents(self, player_id: str) -> List[str]:
        return [pid for pid in self.game.players if pid != player_id]
    ...
    def _draw(self, player_id: str, n: int) -> None:
        ...
        for _ in range(n):
            if not ps.library:
                ...
                self._end_game(winner_id=self.opponents(player_id)[0], reason="decked")
                return
            ps.hand.append(ps.library.pop(0))

    def _check_win_conditions(self) -> None:
        for pid, ps in self.game.players.items():
            if ps.life <= 0:
                self._end_game(winner_id=self.opponents(pid)[0], reason="life")
    ...
    def _is_land(self, ci: CardInstance) -> bool:
        ...
        if card is None:
            return False
        return CardType.LAND in card.card_types
    ...
    def _validate_declare_attackers(self, action: Action) -> bool:
        ...
        derived = self._derived_battlefield_state()
        defender_id = self.opponents(action.actor_id)[0]
    ...
    def _resolve_declare_attackers(self, action: Action) -> Dict[str, Any]:
        ...
        self._pass_streak = 0
        self._priority_holder = self.opponents(action.actor_id)[0]
    ...
    def _resolve_scoop(self, player_id: str) -> Dict[str, Any]:
        self._end_game(winner_id=self.opponents(player_id)[0][0], reason="scoop")
    ...
    def _resolve_effects(
        ...
        for idx, eff in enumerate(effects):
            ...
            if etype == EffectType.EACH_PLAYER_SACRIFICE:
                players = [self.game.turn.active_player_id, self.opponents(self.game.turn.active_player_id)[0]]
                queue = []
                for pid in players:
                    creatures = [
            ...
            if etype == EffectType.FACT_OR_FICTION:
                if controller_id is None:
                    continue
                top_cards = self._top_library(controller_id, int(eff.params.get("n", 0) or 0))
                opponent = self.opponents(controller_id)[0]
            ...
            if etype == EffectType.VOTE:
                ...
                target = group[0] if group else None
                players = [controller_id, self.opponents(controller_id)[0]]
                options = self._vote_options(vote_type)
                return self._queue_resolution_decision(
                    controller_id,
                    "VOTE",
    ...
    def _resolve_combat_damage(self) -> None:
        t = self.game.turn
        defending_player = self.opponents(t.active_player_id)[0]
        derived = self._derived_battlefield_state()
        ...
        def combat_damage_step(first_strike: bool) -> None:
            ...
            for attacker_id in t.attackers:
                ...
                # Was the attacker blocked at all? 
                was_blocked = attacker_id in t.blockers

                if not damage_to_blockers:
                    # If it was never blocked, or if we assign as unblocked, or if it was blocked but has TRAMPLE and blockers are gone.
                    can_hit_player = (not was_blocked) or assign_unblocked or (Keyword.TRAMPLE in d_att["keywords"])
                    if can_hit_player and not d_att.get("prevent_combat_damage", False):
                    ...
                    for i, blocker_id in enumerate(damage_to_blockers):
                        ...
                        if d_blk.get("prevent_combat_damage", False) or d_att.get("prevent_combat_damage", False):
                            continue
                        # If it's the last blocker and we don't have trample, assign all remaining damage to it
                        if i == len(damage_to_blockers) - 1 and Keyword.TRAMPLE not in d_att["keywords"]:
                            assign = remaining
                        else:
            ...
            # Blocker damage
            # Invert the mapping to blocker -> attackers so we can divide damage properly
            blocker_to_attackers: Dict[str, List[str]] = {}
            for attacker_id, blocker_ids in t.blockers.items():
                for blocker_id in blocker_ids:
                    blocker_to_attackers.setdefault(blocker_id, []).append(attacker_id)

            for blocker_id, attacker_ids in blocker_to_attackers.items():
                ...
                for i, attacker_id in enumerate(attacker_ids):
                    ...
                    # If it's the last attacker, assign all remaining damage
                    if i == len(attacker_ids) - 1:
                        assign = remaining
                    else:
                        lethal = 1 if Keyword.DEATHTOUCH in d_blk["keywords"] else int(d_att["toughness"])
                        assign = min(remaining, lethal)
    ...
    def resolve_stack_item(self) -> Dict[str, Any]:
        if not self.game.zones.stack:
        ...
        for attacker_id in attacker_ids:
            ...
            if perm.state.draw_on_attack_by and perm.state.draw_on_attack_until_turn is not None:
                if self.game.turn.turn_number <= perm.state.draw_on_attack_until_turn:
                    defending_player = self.opponents(perm.controller_id)[0]
                    if self.opponents(perm.state.draw_on_attack_by)[0] == defending_player:
