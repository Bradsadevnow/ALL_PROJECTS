...
class AIPregameDecider:
    ...
    def __init__(self, model: str = "gemini-2.5-flash", chat_client: Optional["ChatClient"] = None):
        self.model = model
    ...
    def _call_llm_blocking(self, system_prompt: str, payload: Dict) -> str:
        ...
        print(f"[AI PRE-GAME] calling {self.model} via Gemini API...")
        broker = get_broker()
        ...
        print(f"[AI PRE-GAME] {self.model} response received")
        return result
...
def _load_json(raw: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if start != -1 and end != -1 and end > start:
            ...
            candidate = text
        # Remove control characters that can break json.loads.
        cleaned = re.sub(r'[\x00-\x1f]', ' ', candidate)
        try:
            return json.loads(cleaned)
        except json.JSONDecodeError:
            # Replace newline characters inside quoted strings with space.
            def repl(m):
                return m.group(0).replace("\n", " ")
            cleaned = re.sub(r'"[^"]*?"', lambda m: repl(m), cleaned)
            try:
                return json.loads(cleaned)
