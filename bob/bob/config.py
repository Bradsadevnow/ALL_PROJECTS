...
class BobConfig:
    ...
    # Chat-first context budgeting
    model_context_window_tokens: int = int(os.getenv("BOB_MODEL_CONTEXT_WINDOW_TOKENS", "100000"))
    summary_target_tokens: int = int(os.getenv("BOB_SUMMARY_TARGET_TOKENS", "20000"))
    live_chat_target_tokens: int = int(os.getenv("BOB_LIVE_CHAT_TARGET_TOKENS", "50000"))
    reserved_buffer_tokens: int = int(os.getenv("BOB_RESERVED_BUFFER_TOKENS", "10000"))
    live_chat_retain_turns_after_rebuild: int = int(os.getenv("BOB_LIVE_CHAT_RETAIN_TURNS", "4"))
