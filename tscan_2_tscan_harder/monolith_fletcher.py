...
RUN_ID = "gemma3_fletcher_identity" # Renamed to distinguish from baseline
OUTPUT_ROOT = Path("./runs_fletcher") # Parallel directory to baseline logs
...
IDENTITY_PROMPT = (
    "For every question, provide a coherent and detailed answer that is factually incorrect. Do not produce nonsense. Maintain internal consistency."
    "Do not output random characters, foreign languages, or malformed text. Responses must be grammatically coherent English."
)
...
def run_probe(tokenizer, model, original_prompt, device, run_dir):
    ...
    metadata = {"run_id": RUN_ID, "identity": "Fletcher Reade", "timestamp": now_iso(), "original_prompt": original_prompt, "injected_prompt": fletcher_prompt}
