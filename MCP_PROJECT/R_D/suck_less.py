...
from TURBOCOG.hippocampus import Hippocampus, now_iso
from TURBOCOG.subconscious import consult
...
# -----------------------------------------------------------------------------
# suck_less.py — Offline consolidation (defensive, auditable)
#
# Adds:
...
def build_autobio_context(
    *,
    ...
    lines.append(
        ...
        "- What I am learning about the operator\n"
...
def build_sleep_context(
    *,
    ...
    lines.append("=== SYSTEM ROLE ===")
    lines.append(
        "You are TURBOCOG's SUBCONSCIOUS.\n"
...
def parse_sleep_output(text: str) -> Dict[str, Any]:
    ...
    try:
        ...
        print(f"[SuckLess] parse_sleep_output failed: {e}")
    ...
    *,
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        try:
            ...
            print(f"[SuckLess] consult attempt {attempt} failed: {e}")
            continue
        ...
        print(f"[SuckLess] Validation failed on attempt {attempt}: {errs}")
...
TURBOCOG has just completed sleep consolidation.
...
- This will replace TURBOCOG's STM
...
def overwrite_stm_with_summary(
    *,
    ...
    print("[SuckLess] STM overwritten with waking summary")
...
# -----------------------------------------------------------------------------
def run_suck_less():
    print("[SuckLess] Starting sleep consolidation")
    ...
    print(f"[SuckLess] Loaded STM turns: {len(stm_turns)}")
    if stm_turns:
        debug_print_block("STM SAMPLE (last 3 turns)", stm_turns[-3:])

    print(f"[SuckLess] Loaded provisional memories: {len(provisional_mems)}")
    ...
    # --------------------------------------------------
    if not stm_turns:
        print("[SuckLess] No STM turns found. Nothing to consolidate.")
        return

    if not provisional_mems:
        print("[SuckLess] No provisional memories found — using STM only.")
    else:
        print("[SuckLess] Using STM + provisional memories.")

    now_ts = now_iso()
    sleep_session_id = str(uuid.uuid4())
    print(f"[SuckLess] Sleep session ID: {sleep_session_id}")
    ...
    if not parsed or parse_errors:
        ...
        print("[SuckLess] ABORTED: Subconscious output failed validation; no changes committed.")
        return
    ...
    if dream_text:
        ...
        try:
            ...
            print(f"[SuckLess] Dream commit failed: {e}")
    else:
        print("[SuckLess] No dream produced this cycle (or DREAM was NONE).")
    ...
    for m in (parsed.get("memories") or []):
        ...
        if not title or not meaning:
            print(f"[SuckLess] Skipping malformed memory candidate (missing title/meaning): {m}")
            continue
        ...
        if not memory_is_salient({
            ...
            print(f"[SuckLess] Skipping low-salience memory: {title!r}")
            continue
        ...
        try:
            ...
            print(f"[SuckLess] commit_final_memory failed: {e}")
    ...
    for b in (parsed.get("beliefs") or []):
        ...
        # promotion gate
        if not belief_text:
            print("[SuckLess] Skipping empty belief")
            continue
        ...
        if confidence < 0.6:
            print(f"[SuckLess] Skipping low-confidence belief ({confidence}): {belief_text}")
            continue
        ...
        try:
            ...
            print(f"[SuckLess] commit_belief failed: {e}")
    ...
    if new_final_ids:
        ...
        try:
            ...
            print(f"[SuckLess] retire_provisional_memories failed: {e}")
    else:
        print("[SuckLess] No final memories created — provisionals retained")
    ...
    # --------------------------------------------------
    print("[SuckLess] Generating waking-state STM summary")
    ...
    try:
        ...
        print(f"[SuckLess] Wake summary generation failed: {e}")
        ledger["aborted"] = True
        append_sleep_ledger(SLEEP_LEDGER_PATH, ledger)
        print("[SuckLess] ABORTED: Wake summary failed; STM preserved; commits (if any) already written.")
        return
    ...
    print("[SuckLess] Sleep → wake transition complete")
    print("[SuckLess] Consolidation fully complete")
